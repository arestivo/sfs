...
public class Rail extends Conveyor {
	...
	public Point2D.Double getSensorPosition(int i) {
		if (getOrientation() == Orientation.VERTICAL)
			return getSensorPosition(i, railPosition, 0);
		else
			return getSensorPosition(i, 0, railPosition);
	}
	...
	public void doStep(boolean conveyorBlocked) {
		...
		if (isRailMovingLeft() && !isRailMovingRight()) {
			railPosition -= speed;
			movedLeft = true;
		}
		if (isRailMovingRight() && !isRailMovingLeft()) {
			railPosition += speed;
			movedRight = true;
		}

		if (railPosition < -railSize / 2) {
			railPosition = -railSize / 2;
			forcing = true;
			movedLeft = false;
		}
	...
	public Rectangle getBounds() {
		double pixelSize = getFactory().getPixelSize();
		int x = getOrientation() == Orientation.VERTICAL ? (int) (getCenterX() / pixelSize - width / 2 / pixelSize + railPosition / pixelSize) : (int) (getCenterX() / pixelSize - length / 2
		        / pixelSize);
		int y = getOrientation() == Orientation.VERTICAL ? (int) (getCenterY() / pixelSize - length / 2 / pixelSize) : (int) (getCenterY() / pixelSize - width / 2 / pixelSize + railPosition
		        / pixelSize);
