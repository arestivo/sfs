...
public class Portal3D extends Facility {
	...
	public void paintTop(Graphics g) {
		...
		for (int i = 0; i < sensorsx; i++)
			paintLight(g, true, i, getDigitalIn(i), 1);
		for (int i = 0; i < sensorsy; i++)
			paintLight(g, true, i, getDigitalIn(sensorsx + i), 2);
	...
	public void doStep(boolean conveyorBlocked) {
		...
		if (getDigitalOut(0) && !getDigitalOut(1) && positionz == 1)
			positionx += speed;
		if (getDigitalOut(1) && !getDigitalOut(0) && positionz == 1)
			positionx -= speed;
		if (getDigitalOut(2) && !getDigitalOut(3) && positionz == 1)
			positiony += speed;
		if (getDigitalOut(3) && !getDigitalOut(2) && positionz == 1)
			positiony -= speed;
		if (getDigitalOut(4) && !getDigitalOut(5))
			positionz += speed / 4;
		if (getDigitalOut(5) && !getDigitalOut(4))
			positionz -= speed / 4;
		...
		if (positionz < 0) {
			forcing = true;
			positionz = 0;
			setDigitalIn(sensorsx + sensorsy, true);
		} else
			setDigitalIn(sensorsx + sensorsy, false);
		if (positionz > 1) {
			forcing = true;
			positionz = 1;
			setDigitalIn(sensorsx + sensorsy + 1, true);
		} else
			setDigitalIn(sensorsx + sensorsy + 1, false);

		for (int i = 0; i < sensorsx; i++)
			if (Math.abs(getSensorPositionX(i) - getCenterX() + width / 2 - positionx) < sensorradius)
				setDigitalIn(i, true);
			else
				setDigitalIn(i, false);

		for (int i = 0; i < sensorsy; i++)
			if (Math.abs(getSensorPositionY(i) - getCenterY() + height / 2 - positiony) < sensorradius)
				setDigitalIn(i + sensorsx, true);
			else
				setDigitalIn(i + sensorsx, false);
		...
		if (block != null || isBlockPresent())
			setDigitalIn(sensorsx + sensorsy + 2, true);
		else
			setDigitalIn(sensorsx + sensorsy + 2, false);
	...
	private Block getCurrentBlock() {
		if (positionz == 0) {
			...
			for (Block block : blocks) {
				...
				if (block.getDistanceTo(sp.x, sp.y) < sensorradius)
					return block;
			}
		}
		return null;
